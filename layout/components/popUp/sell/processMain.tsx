...
export const Item: FC<OfferI> = ({steamOfferId, offerId, bot, status, statusCode}) => {
    let statusTrade
    let stylesTrade
    // console.log(status && [6, 7, 8, 10].includes(statusCode), statusCode, status)
...
export const Process: FC = () => {
    ...
    const createTrade = async () => {
        ...
        const trade_data = {
            ...
            amount: store.amount,
            price: item.sell,
            totalPrice: item.sell * store.amount,
    ...
    const PendingTrade = async () => {
        ...
        while (!finish){
            ...
            if (!response.data.status && response.data.err === "This trade has already ended"){
                ...
                const item = store_product[store.activeItem]
                if (item.symbol === "$") {
                    store_user.setUserBalanceUSD(store_user.balance_usd + (item.sell * store.amount))
                }
                else {
                    store_user.setUserBalanceRUB(store_user.balance_rub + (item.sell * store.amount))
                }
